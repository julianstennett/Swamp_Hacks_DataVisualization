...
from swamp_hacks_AlgalBlooms import data
...
class App(customtkinter.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set default values
        self.map_widget.set_address("California")
    ...
    def DataToMap(self):
        ...
        for i in range(self.a, self.b):
            if (data[i]['Latitude'] != "NA") and (data[i]['Longitude'] != "NA"):
                self.set_marker_event([float(data[i]['Latitude']), float(data[i]['Longitude'])])
        if self.b == 1300:
            self.a, self.b = 0, 100
        else:
            self.a += 100
    ...
    def set_marker_event(self, current_position=[]):
        if current_position == []:
